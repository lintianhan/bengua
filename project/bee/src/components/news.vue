    ...
    <div :style="{ 'border-radius': '6px',margin: '-5px 16px',background: '#fff', }">
      ...
      >
        ...
        <div :style="{'margin-button': '6px', padding: '24px','border-radius': '6px',}">
          <p>新闻标题:</p>
          <a-input style="width: 200px" v-model="item.term" placeholder="新闻标题" type="text" />
...
<script>
import { getNewsMsgListForPage } from "../api/api";
...
export default {
  ...
  methods: {
    getList() {
      ...
      getNewsMsgListForPage(this.payload).then((res) => {
